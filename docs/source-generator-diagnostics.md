...
- 参数不能包含 `ref` / `out` / `in`。
- 在支持 `IParsable<TSelf>` 的目标框架上，参数类型必须可按该接口解析；`StackT`、`Variables`、`ExtraParameters` 这三类特殊参数除外。
...
| `NSPGEN102` | Warning | 参数类型不受支持；请确保该类型实现了 `System.IParsable<TSelf>`。                                                                                   |
| `NSPGEN103` | Info    | 带 `ToVariableAttribute` 的方法不能返回 `void`。                                                                                                                            |
| `NSPGEN121` | Warning | 入口点与其他导出冲突。                                                                                                                                                      |
| `NSPGEN122` | Warning | 入口点格式字符串无效。                                                                                                                                                      |
| `NSPGEN123` | Warning | 入口点名称无效，不是合法标识符或包含不支持导出的字符。                                                                                                                      |
